...
export default function Home() { const [messages, setMessages] = useState([]); const [input, setInput] = useState(''); const [isTyping, setIsTyping] = useState(false); const [imageFile, setImageFile] = useState(null); const [drawerOpen, setDrawerOpen] = useState(false); const [sessions, setSessions] = useState([]); const [sessionId, setSessionId] = useState(() => Date.now()); const inputRef = useRef(null); const endOfMessagesRef = useRef(null);

useEffect(() => { const storedSessions = JSON.parse(localStorage.getItem('chat_sessions') || '{}'); setSessions(Object.keys(storedSessions)); const current = storedSessions[sessionId] || [{ role: 'assistant', content: 'Halo, saya Smile AI!' }]; setMessages(current); }, [sessionId]);

useEffect(() => { const stored = JSON.parse(localStorage.getItem('chat_sessions') || '{}'); stored[sessionId] = messages; localStorage.setItem('chat_sessions', JSON.stringify(stored)); }, [messages, sessionId]);
...
const handleNewSession = () => { const newId = Date.now(); setSessionId(newId); setDrawerOpen(false); };

return ( <> <Head> <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600&display=swap" rel="stylesheet" /> </Head> <div className="flex h-screen bg-gray-900 text-white font-sans" style={{ fontFamily: 'Inter, sans-serif' }}> {/* Sidebar drawer */} {drawerOpen && ( <div className="fixed inset-y-0 left-0 w-64 bg-gray-800 p-4 border-r border-gray-700 z-50"> <button
onClick={handleNewSession}
className="w-full text-left bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded-md font-medium"
> + Obrolan Baru </button> <hr className="my-4 border-gray-600" /> <div className="space-y-2"> {sessions.map((id) => ( <button key={id} onClick={() => { setSessionId(id); setDrawerOpen(false); }} className={w-full text-left px-4 py-2 rounded-md ${id == sessionId ? 'bg-gray-700' : 'hover:bg-gray-600'}} > Session {id} </button> ))} </div> </div> )}

<div className="flex-1 flex flex-col">
      {/* Header */}
      <header className="bg-gray-800 px-4 py-3 border-b border-gray-700 flex justify-between items-center">
        <button onClick={() => setDrawerOpen(!drawerOpen)} className="text-white p-1">
          <Menu size={22} />
        </button>
